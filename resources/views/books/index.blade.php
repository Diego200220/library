                    ...
                    <tr>
                        ...
                        <td style="background-color: #e1d1a7">
                            <button type="button" class="btn btn-success" data-bs-toggle="modal"
                                    data-bs-target="#edit-book-modal{{ $Book->id }}">
                                Editar
                            </button>
...
@foreach ($books as $Book)
<!-- Modal para Editar -->
<div class="modal fade" id="edit-book-modal{{ $Book->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<!-- Modal para Eliminar -->
<div class="modal fade" id="delete{{ $Book->id }}" tabindex="-1" aria-labelledby="deleteLabel{{ $Book->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
    ...
    <script>
        ...
        */
        ...
        function getBookInfo(bookId) {
            fetch('{{ route("books.update", ":id") }}'.replace(':id', bookId)) // consultar api
                .then(response => response.json()) // obtener los datos de un libro
                .then(function (response) {
                    // actualizar la acción del formulario
                    document.querySelector('#edit-book-modal form').action = '{{ route("books.update", ":id") }}'.replace(':id', bookId);

                    // llenar los inputs del modal
                    document.querySelector('#edit-book-modal input[name="title"]').value = response.data.book.title;
                    document.querySelector('#edit-book-modal input[name="author"]').value = response.data.book.description;
                    document.querySelector('#edit-book-modal input[name="library_id"]').value = response.data.book.description;
                    document.querySelector('#edit-book-modal input[name="classification_id"]').value = response.data.book.description;
                    // ...
        ...
        const editBookModal = new bootstrap.Modal('#delete-book-modal');
        /*
        Definir una función -> consultar api -> obtener los datos de un libro -> actualizar la acción del formulario -> llenar los inputs del modal -> mostrar el modal
        */
        // Definir una función
        function getBookInfo(bookId) {
            fetch('{{ route("books.update", ":id") }}'.replace(':id', bookId)) // consultar api
                ...
                .then(function (response) {
                    ...
                    // llenar los inputs del modal
                    document.querySelector('#edit-book-modal input[name="title"]').value = response.data.book.title;
                    document.querySelector('#edit-book-modal input[name="description"]').value = response.data.book.description;
                    // ...
